...
| 参数名           | 类型      | 必需               | 默认值     | 描述                                       |
| ------------- | ------- | ---------------- | ------- | ---------------------------------------- |
| expire_time   | int     | 否                | 259200(72小时)  | 消息离线存储时间（单位为秒），最长存储时间72小时<br>1）若设置为0，则表示实时消息<br>2）若设置0&ltexpire_time&lt180s，则系统会重置为180s<br> 3）若设置expire_time>=180s，按实际设置时间存储，最长72小时<br>|
