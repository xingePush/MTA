...
| xg_media_resources    | array  | 无    | 否    | 富媒体元素地址，建议小于5个   （仅限SDK4.2.0及以上版本使用）              |
...
| 参数名           | 类型      | 必需               | 默认值     | 描述                                       |
| ------------- | ------- | ---------------- | ------- | ---------------------------------------- |
| expire_time   | int     | 否                | 259200(72小时)  | 消息离线存储时间（单位为秒）,最长72小时<br>1）若expire_time=0，则表示实时消息<br>2）若0&ltexpire_time&lt180s，则系统会重置为180s<br>3）若expire_time>=180s，按实际设置时间存储，最长72小时 |
