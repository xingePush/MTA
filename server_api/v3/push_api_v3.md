...
| 参数名           | 类型      | 必需               | 默认值     | 描述                                       |
| ------------- | ------- | ---------------- | ------- | ---------------------------------------- |
| expire_time   | int     | 否                | 259200  | 1.消息离线存储时间（单位为秒），最长存储时间3天<br>2.若不设置或者设置为0，则取默认值（3天）<br>3.若设置0&ltexpire_time&lt180，则系统会重置为180s<br>4.若设置离线存储时间>=180s，按实际设置时间存储，最长72小时 <br>5.第三方通道离线保存消息不同厂商标准不同 |
