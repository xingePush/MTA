...
| 参数名           | 类型      | 必需               | 默认值     | 描述                                       |
| ------------- | ------- | ---------------- | ------- | ---------------------------------------- |
| expire_time   | int     | 否                | 259200  | 消息离线存储时间（单位为秒）<br>1）最长存储时间3天，若设置为0，则默认值（3天）<br>2）建议取值区间[600, 86400x3]<br>3）第三方通道离线保存消息不同厂商标准不同 |
...
| environment   | string  | 否                | product | 用户指定推送环境，仅限iOS平台推送使用<br>1）product： 推送生产环境<br>2）dev： 推送开发环境 |
