...
| 参数名           | 类型      | 必需               | 默认值     | 描述                                       |
| ------------- | ------- | ---------------- | ------- | ---------------------------------------- |
| expire_time   | int     | 否                | 259200(72小时)  | 消息离线存储时间（单位为秒）,最长72小时<br>1）若expire_time=0，则表示实时消息<br>2）若expire_time大于0且小于800s，则系统会重置为800s<br>3）若expire_time>=800s，按实际设置时间存储，最长72小时 |
