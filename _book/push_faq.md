...
* 管理台和调用服务端API创建的推送，推送明细统一限制查询3个月的记录。若要查3个月以前数据，需要联系[技术支持](http://xg.qq.com/app/ctr_feedback/index）)来查
...
**问: 信鸽4.X版本注册时，出现`Server response error code:404, error:{"ret":-1, "msg":"invalid appkey"}`**
```
答: 信鸽SDK内部集成了MTA SDK，日志是与MTA相关的，不影响信鸽注册。
```

**问: 信鸽4.X版本注册时，出现` D/XINGE: [TPush] channelId没有初始化；`**
```
答: 信鸽SDK内部的日志，不影响信鸽注册。
```
**问: 管理台无法自行编辑标签？**


```
答: 目前管理台的标签推送规则是：终端绑定标签后，后台定时刷新数据，将标签展示到管理台上，故无法自行编辑，使用RST API可以自行发送。
```
...
（1）小米手机的通知栏消息是展示最新的一条，如果每条都要展示需要设置n_id。
（2）消息广播被手机管家屏蔽了，关闭管家。
